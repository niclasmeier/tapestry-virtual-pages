...
public class AppModule
{
	...
	{
		binder.bind(VirtualPageData.class, VirtualPageDataImpl.class).scope(ScopeConstants.PERTHREAD);
	 ...
	 * within the RequestHandler service pipeline, which is built from the RequestHandler service configuration. Tapestry
	 * IoC is responsible for passing in an appropriate Logger instance. Requests for static resources are handled at a
	 * higher level, so this filter will only be invoked for Tapestry related requests.
	 * <p>
	 ...
	 * selects the desired service by type, but only from the same module. Without @Local, there would be an error due to
	...
	public ComponentClassResolver decorateComponentClassResolver(ComponentClassResolver toDecorate, Logger logger,
		PageTemplateLocator templateLocator, ThreadLocale threadLocale, VirtualPageData virtualPageData)
	{
		return new DecoratedComponentClassResolver(toDecorate, logger, templateLocator, threadLocale, virtualPageData);
	}

	public DecoratedComponentInstantiatorSource decorateComponentInstantiatorSource(
		ComponentInstantiatorSource toDecorate, VirtualPageData virtualPageData)
	{
		return new DecoratedComponentInstantiatorSource(toDecorate, virtualPageData);
	}
}
